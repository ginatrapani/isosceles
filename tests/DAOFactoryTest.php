...
class DAOFactoryTest extends IsoscelesUnitTestCase {
    ...
    public function testDAODBType() {
        ...
        $this->assertEquals($type, 'mysql', 'should default to mysql');

        Config::getInstance()->setValue('db_type', 'some_sql_server');
        $type = DAOFactory::getDBType();
        $this->assertEquals($type, 'some_sql_server', 'is set to some_sql_server');
    }
    ...
    public function testGetTestDAO() {
        ...
        try {
            ...
            $this->assertRegExp('/No DAO mapping defined for: NoSuchDAO/', $e->getMessage(), 'no dao mapping');
        }
        ...
        try {
            ...
            $this->assertRegExp("/No db mapping defined for 'TestDAO'/", $e->getMessage(), 'no dao db_type mapping');
        }

        // valid mysql test dao
        Config::getInstance()->setValue('db_type', 'mysql');
        $test_dao = DAOFactory::getDAO('TestDAO');
        $this->assertInstanceOf('TestMySQLDAO', $test_dao);
        $data_obj = $test_dao->selectRecord(1);
        $this->assertNotNull($data_obj);
        $this->assertEquals($data_obj->test_name, 'name1');
        $this->assertEquals($data_obj->test_id, 1);

        // valid fuax test dao
        Config::getInstance()->setValue('db_type', 'faux');
        $test_dao = DAOFactory::getDAO('TestDAO');
        $this->assertInstanceOf('TestFauxDAO', $test_dao);
        $data_obj = $test_dao->selectRecord(1);
        $this->assertNotNull($data_obj);
        $this->assertEquals($data_obj->test_name, 'Mojo Jojo');
        $this->assertEquals($data_obj->test_id, 2001);
    }
}
